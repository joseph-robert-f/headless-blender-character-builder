...
required='MINIO_ROOT_USER MINIO_ROOT_PASSWORD HBCB_STORAGE_API_ACCESS_KEY HBCB_STORAGE_API_SECRET_KEY HBCB_STORAGE_WORKER_ACCESS_KEY HBCB_STORAGE_WORKER_SECRET_KEY HBCB_STORAGE_BUCKET HBCB_DEPLOYMENT_NAMESPACE'
...
if [ "$HBCB_STORAGE_API_ACCESS_KEY" = "$HBCB_STORAGE_WORKER_ACCESS_KEY" ] || \
   ...
   [ "$HBCB_STORAGE_WORKER_ACCESS_KEY" = "$MINIO_ROOT_USER" ]; then
  printf '%s\n' 'MINIO_INIT: storage identities must be distinct' >&2
  exit 4
fi
case "$MINIO_ROOT_USER" in
  hbcb_api|hbcb_api_*|hbcb_worker|hbcb_worker_*)
...
# hbcb_api* and hbcb_worker* prefixes are reserved to this project.  This never
# removes buckets, objects, or object versions.
