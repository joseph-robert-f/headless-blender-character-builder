...
- `.github/` contains CODEOWNERS, issue/pull-request templates, and JSON-form YAML workflows with read-only permissions, full-SHA-pinned checkout, no persisted credentials, no `pull_request_target`, and no self-hosted runner; the scheduled/manual dependency audit reports synchronized-pin, maintenance, and vulnerability findings without opening pull requests;
