# ARCHIVED — Open-Source Headless Blender Character Builder (original v0.1 plan)

Status: **superseded. Kept for reference.**

Last updated: **July 31, 2026**. Archived August 2026.

> **This document is no longer the plan.** The current plan is
> [`PLAN.md`](../PLAN.md) in the repository root. Read that first.
>
> **What happened to it.** Milestones M0–M2 (work packages G0–G4) — the
> deterministic builder — were implemented, with deliberate changes recorded in
> [docs/REVIEW.md](REVIEW.md). Milestones M3–M5 (G5–G8: FastAPI, Postgres,
> Redis, MinIO, Compose, the VPS package) were deferred out of v0.1 so a usable
> release did not sit behind infrastructure that produces no geometry.
>
> **What is still worth reading here**, because the current plan defers to it
> rather than restating it:
>
> - §6 System architecture and §7 Public API v1 — the intended service design.
> - §13 Security model — the isolation and worker requirements.
> - §14 Licensing and IP plan.
> - §16 Out of scope.
>
> **What is now wrong here.** The request carries a `print_profile` that drives
> generation as well as QA, rather than the fixed 1.2 mm / 2.0 mm constants in
> §8; `CharacterSpec` has no `seed` field; there are additional output and
> quality profiles; and `PLAN.md` now adds a build `intent` so that digital-only
> models are a first-class path. Where this document and the code disagree, the
> code and `schemas/` are authoritative.
