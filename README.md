...
> **Status: G0–G5 passed; keyless builder and service foundations implemented.** The deterministic Blender/container path plus Postgres schema, transactional-outbox semantics, Redis and versioned-artifact adapters, bearer/idempotency policy, and generated role-scoped local credentials are implemented and tested. The HTTP API, worker supervisor, Compose stack, VPS package, and release automation remain pending. This is not production-ready yet.
...
The secondary path will add an asynchronous self-hosted service through Docker Compose. Its local credentials can already be generated safely:
...
G5 adds service foundations without changing that builder revision. PostgreSQL owns state and a transactional queue outbox; Redis messages contain only build IDs. Artifacts use generated attempt-scoped keys, version-pinned reads, exact-body SHA-256 verification, and manifest-last success publication. Generated local configuration separates API, worker, migrator, and administrative credentials. These contracts passed 55 focused tests plus a real PostgreSQL 16.9 migration/constraint gate, but no HTTP endpoint or Compose service is available yet.
